...
- 新增统一输入消息闭环，支持文本、URL、图片、语音等内容以统一模型进入系统。
- URL 类型消息会在处理阶段自动抓取并整理网页内容，再写入知识库与召回链路。
- 新增 `syncTargets`、`syncStatus`、`externalReferencesJson` 等字段，可为消息追加外部同步状态。
- 已实现 `feishu` 同步，可将处理后的内容写入飞书文档或飞书知识库节点；当前请求中的 `syncTargets` 仅支持 `feishu`。
- 新增烟雾验证脚本和独立 smoke 配置，便于验证输入接入、知识入库和飞书同步闭环。

详细说明见：

- [输入消息闭环说明](docs/input_messages.md)
- [飞书同步说明](docs/feishu_sync.md)
- [快速开始中的输入闭环章节](docs/get_started/quick_start.md)
